...
function BmpCodec.write(BaseStream, params)
    local bs = BinaryStream:new(BaseStream);

--print("FILESIZE: ", params.filesize);
--print("PIXEL OFF:", params.pixeloffset);

	-- Write File Header
    bs:writeByte(string.byte('B'))
    bs:writeByte(string.byte('M'))
    bs:writeInt32(params.filesize);
    bs:writeInt16(0);
    bs:writeInt16(0);
    bs:writeInt32(params.pixeloffset);

    -- Bitmap information header
    bs:writeInt32(40);
    bs:writeInt32(params.width);    -- dibsec.Info.bmiHeader.biWidth
    bs:writeInt32(-params.height);   -- dibsec.Info.bmiHeader.biHeight
    bs:writeInt16(1);               -- dibsec.Info.bmiHeader.biPlanes);
    bs:writeInt16(params.bitcount); -- dibsec.Info.bmiHeader.biBitCount);
    bs:writeInt32(BI_RGB);               -- dibsec.Info.bmiHeader.biCompression);
    bs:writeInt32(params.pixelarraysize);               -- dibsec.Info.bmiHeader.biSizeImage);
    ...
    BaseStream:writeBytes(params.data, params.pixelarraysize, 0);
end
